...
export function NobetScheduler() {
  ...
  const removeAssistant = useCallback((assistantId: IAssistant['id']) => {
    ...
    const assistantSelectedDays = assistantList.find(assistant => assistant.id !== assistantId)?.selectedDays.days;
  ...
  const setAssistantProps = useCallback((assistantId: IAssistant['id'], props: Partial<IAssistant>) => {
    setAssistantList((prevState) =>
      prevState.map(assistant => assistant.id === assistantId ? { ...assistant, ...props } : assistant)
    );
  }, []);
          ...
          sectionList.map(section => (
            {
              ...
              Cell: ({ row }: { row: MRT_Row<IAssistant> }) => (
                <SectionCellRenderer assistant={row.original}
                                     section={section}
                                     setAssistantProps={setAssistantProps}
                />
              )
